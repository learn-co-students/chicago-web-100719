...
* The `:players` key points to an array of players hashes containing their stats. The values for each player's names and their stats can be found in the table below. The stats keys should be formatted like this:
  * `:player_name`
  * `:number`
  * `:shoe`
  * `:points`
